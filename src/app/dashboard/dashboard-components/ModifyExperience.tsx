...
const ModifyExperience: React.FC<ModifyExperienceProps> = ({ workExperience, purpose }) => {
  const {
    register,
    handleSubmit,
    reset,
    ...
    if (workExperience) {
      reset({
        ...
        startDate: workExperience.startDate
          ? new Date(workExperience.startDate).toISOString().split("T")[0]
          : "",
        endDate: workExperience.endDate && !workExperience.isCurrent
          ? new Date(workExperience.endDate).toISOString().split("T")[0]
          : "",
      });
    }
  ...
  useEffect(() => {
    ...
    if (resData && resData.success) {
      toast(new Toast("Success", `Experience ${purpose.toLowerCase()}ed successfully`));
      timeout = setTimeout(() => {
