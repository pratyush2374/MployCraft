const getFresherResume = (data: any): string => {
            ...
            <div class="max-w-[200mm] mx-auto">
                ...
                <header class="border-b pb-2 mb-3">
                    <div class="flex justify-between items-start">
                        <div>
                            <h1 class="text-3xl font-bold mb-1">${data.fullName || ''}</h1>
                            <div class="text-gray-600">
                                <a href="mailto:${data.email || ''}" class="hover:text-blue-600">${data.email || ''}</a>
                ...
                <section class="mb-3">
                    ...
                    <p class="text-gray-700 leading-tight text-[11px]">
                        ${data.professionalSummaryRC || data.professionalSummary || ''}
                    </p>
                </section>
                    ...
                    ${data.education && data.education.length > 0 
                        ? data.education.map((edu: any) => 
                            `<div class="flex justify-between">
                                <div>
                                    <h3 class="font-semibold">${edu.instituteName || ''}</h3>
                                    <p>${edu.major || ''} (${formatDate(edu.startDate)}-${formatDate(edu.endDate)})</p>
                                </div>
                                <p class="text-gray-600">${edu.scoreType || ''} - ${edu.score || ''}</p>
                            </div>`
                        ).join('')
                ...
                <section class="mb-3">
                    ...
                    ${data.workExp && data.workExp.length > 0 
                        ? data.workExp.map((exp: any) => 
                            `<div class="mb-2 w-[95%] m-auto">
                                <div class="flex justify-between">
                                    <div>
                                        <h3 class="font-semibold">${exp.company || ''}</h3>
                                        <p>${exp.jobTitle || ''} ${exp.location ? `(${exp.location})` : ''}</p>
                                    </div>
                                    <p class="text-gray-600">${formatDate(exp.startDate)} - ${exp.isCurrent ? "Present" : formatDate(exp.endDate)}</p>
                                </div>
                        ...
                        <ul class="list-disc pl-4 space-y-1 text-gray-700 text-[11px]">
                            ${data.certifications.map((cert: any) => 
                                `<li>
                                    ...
                                    ${cert.additionalInformation ? ` (${cert.additionalInformation})` : ''}
                                </li>`
                            ).join('')}
                        </ul>
                    </section>`
...
export default getFresherResume;